...
Tags: flipbox, flip box, flip card, image overlay, hover effects
...
**Flipbox** is the most complete flipbox plugin for WordPress. Create beautiful flip boxes, flip cards, and image overlay effects with 29 professionally designed styles (10 free, 19 Pro), 4 flip directions, and 50+ scroll animations.

✨ No coding. Pure CSS3. Smooth, responsive flipbox cards that work everywhere.
...
A flipbox (also called a flip box or flip card) lets you show content on the front and reveal more on the back with a smooth CSS3 animation. Flipbox gives you total control over both sides: icons, images, headings, descriptions, buttons, colors, borders, shadows, and animation speed, all from a clean visual admin panel.

Every flipbox animation is built in pure CSS3, so your pages stay fast.
...
* 🐛 Fix: Style 4 back side content clipped on mobile, height now uses pixel-based minimum instead of aspect-ratio percentage
* 🐛 Fix: Style 4 back side content no longer hidden behind overflow on mobile, overflow adjusted and vertical centering corrected for small screens
* 🐛 Fix: Style 4 background image distorted on mobile, switched to `contain` with centered positioning on mobile while preserving desktop behaviour
